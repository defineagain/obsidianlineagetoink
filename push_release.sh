...
echo "Building project binaries for 0.3.1..."
npm run build
...
echo "Creating GitHub Release 0.3.1..."
if [ -z "$GITHUB_TOKEN" ]; then
    echo "⚠️ Warning: GITHUB_TOKEN environment variable is not set. The gh cli might prompt for authentication."
fi

gh release create 0.3.1 main.js manifest.json styles.css \
    --title "v0.3.1 Refined Marker Handling" \
    --notes "Implements Content Predominance (exporter skips auto-prefixes if manual markers exist) and refined Smart Insertion logic in the Block Editor for Knots, Stitches, and Choices."

if [ $? -eq 0 ]; then
    echo "✅ Release 0.3.1 published successfully!"
